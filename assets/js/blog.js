...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function renderSinglePost(posts, version) {
        ...
        if (post) {
            document.title = `${post.title} - Blog Júpiter Lite`; 
            ...
            // --- LÓGICA DE POSTS RELACIONADOS (NOVO) ---
            // Pega os 3 primeiros posts que NÃO sejam o atual
            const relatedPosts = posts.filter(p => p.id !== post.id).slice(0, 3);
            
            // Gera o HTML dos relacionados
